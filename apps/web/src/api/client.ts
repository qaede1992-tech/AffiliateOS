import type { Affiliate, Campaign, Commission, Conversion, CreateAffiliateRequest, CreateCampaignRequest, CreateOfferRequest, ListResponse, MarketplaceConnectionView, MarketplaceProviderInfo, Offer, TrackingLink, TrackingLinkStats, UpdateCampaignRequest } from "@affiliateos/shared";
...
async function post<T>(path: string, body: unknown): Promise<T> {
  ...
  if (!response.ok) { const error = (await response.json().catch(() => null)) as { message?: string } | null; throw new Error(error?.message ?? `Request failed: ${response.status}`); }
  return response.json() as Promise<T>;
}
