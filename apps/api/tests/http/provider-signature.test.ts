...
test("accepts a valid provider signature within the replay window", () => assert.equal(verifyProviderEventSignature(rawBody, secret, headers, nowMs), true));
test("rejects a tampered body", () => assert.equal(verifyProviderEventSignature(`${rawBody} `, secret, headers, nowMs), false));
test("rejects a wrong secret", () => assert.equal(verifyProviderEventSignature(rawBody, "wrong-secret", headers, nowMs), false));
...
test("rejects an expired timestamp", () => {
  ...
  assert.equal(verifyProviderEventSignature(rawBody, secret, { timestamp: expiredTimestamp, signature: `v1=${expiredSignature}` }, nowMs), false);
});
...
test("rejects timestamps too far in the future", () => {
  ...
  assert.equal(verifyProviderEventSignature(rawBody, secret, { timestamp: futureTimestamp, signature: `v1=${futureSignature}` }, nowMs), false);
});

test("rejects a signature with a mismatched embedded timestamp", () => assert.equal(verifyProviderEventSignature(rawBody, secret, { timestamp, signature: `t=${Number(timestamp) + 1},v1=${signature}` }, nowMs), false));
