...
function inbox() {
  ...
  return {
    events,
    async findByExternalId(account: string, id: string) { return events.get(`${account}:${id}`); },
    ...
    add(account: string, id: string, payload: Record<string, unknown>, eventType = "conversion.created") { events.set(`${account}:${id}`, { affiliateAccountId: account, externalEventId: id, eventType, payload, status: "received", receivedAt: "2026-09-20T00:00:00.000Z" }); }
  };
}
...
test("marks unsupported provider conversion events failed and allows retry after correction", async () => {
  const store = inbox();
  store.add("account-a", "evt-2", { id: "evt-2" }, "order.created");
  const processor = new ProviderEventConversionProcessor(
    new ProviderEventProcessor(store),
    new StaticProviderEventConversionNormalizerRegistry([new GenericProviderConversionNormalizer()]),
    {} as any
