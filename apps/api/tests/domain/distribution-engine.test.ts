...
describe("DistributionEngine", () => {
  ...
  it("skips autonomous scheduling when no publisher is ready", async () => {
    ...
    const result = await engine.scheduleIfReady({ content: created, scheduledAt: "2026-09-21T10:00:00.000Z" });
    assert.equal(result, undefined);
    assert.equal((await contentService.get(created.id)).status, "draft");
  });
  it("schedules autonomously only when an active account and compatible publisher exist", async () => {
    const { contentService, socialAccounts, created } = await setup("tiktok", [account("tiktok-account", "tiktok")]);
    const publisher: SocialPublisher = { supports: (platform) => platform === "tiktok", publish: async () => ({ externalPostId: "post-1" }) };
    const engine = new DistributionEngine(contentService, socialAccounts, [publisher]);
    const result = await engine.scheduleIfReady({ content: created, scheduledAt: "2026-09-21T10:00:00.000Z" });
    assert.ok(result);
