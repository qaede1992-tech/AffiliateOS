...
test("recovery state is retained in feedback snapshots",async()=>{
 ...
 const followUp={...halt,id:"f",clickCount:120,conversionCount:16,conversionRate:16/120,anomaly:"none",anomalyScore:0,observedAt:"2026-01-01T12:00:00.000Z"};
 const snapshots:any[]=[];
 const memory:any={latestByProductAndMarketplace:async()=>halt,recentByProductAndMarketplace:async()=>[halt,followUp],saveIfAbsent:async(s:any)=>{snapshots.push(s);return s}};
 const provider=new AutonomousAnalyticsFeedbackProvider(analytics,memory,()=>now);
