...
export function createApp(services: Services = createInMemoryServices(), options: AppOptions = {}) {
  ...
  app.addHook("preParsing", (request, reply, payload) => isPublicProviderEvent(request.url) ? captureRawBody(request, reply, payload) : payload);
