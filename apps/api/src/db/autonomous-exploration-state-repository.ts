...
const regimeIncrement=(regime: AutonomousDecisionAudit["performanceRegime"]) => regime === "rising" ? { regimeRisingCount: sql`${autonomousExplorationStates.regimeRisingCount}+1` } : regime === "declining" ? { regimeDecliningCount: sql`${autonomousExplorationStates.regimeDecliningCount}+1` } : regime === "volatile" ? { regimeVolatileCount: sql`${autonomousExplorationStates.regimeVolatileCount}+1` } : regime === "stable" ? { regimeStableCount: sql`${autonomousExplorationStates.regimeStableCount}+1` } : {};
 ...
 async applyOptimizationFeedback(): Promise<void> {
  const rows = await this.db.execute(sql`SELECT o.id AS outcome_id, o.action, o.observed_at, o.baseline_metrics, o.evaluation_metrics,
    ...
    WHERE o.status = 'mutated' AND o.evaluated_at IS NOT NULL AND o.evaluation_metrics IS NOT NULL
      ...
      AND NOT (
        o.recovery_episode_id IS NOT NULL
        AND current_recovery.recovery_episode_id IS NOT NULL
        AND o.recovery_episode_id <> current_recovery.recovery_episode_id
      )`);
