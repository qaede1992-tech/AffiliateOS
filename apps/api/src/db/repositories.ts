...
class DrizzleCampaignOfferRepository implements CampaignOfferRepository {
  ...
  async find(campaignId: string, affiliateOfferId: string) { const rows = await this.db.select().from(campaignOffers).where(and(eq(campaignOffers.campaignId, campaignId), eq(campaignOffers.affiliateOfferId, affiliateOfferId))).limit(1); return rows[0] as CampaignOffer | undefined; }
...
const createRepositories = (db: DatabaseExecutor): RepositorySet => ({
  affiliates: new DrizzleRepository(db, affiliates, toAffiliate, (entity: Affiliate) => entity as any),
  offers: new DrizzleRepository(db, offers, toOffer, (entity: Offer) => entity as any),
  conversions: new DrizzleRepository(db, conversions, toConversion, (entity: Conversion) => entity as any),
  commissions: new DrizzleRepository(db, commissions, toCommission, (entity: Commission) => entity as any),
  ...
  campaigns: new DrizzleRepository(db, campaigns, toCampaign, (entity: Campaign) => ({ ...entity, startAt: entity.startAt ?? null, endAt: entity.endAt ?? null } as any)),
  campaignOffers: new DrizzleCampaignOfferRepository(db), trackingLinks: new DrizzleTrackingLinkRepository(db), clicks: new DrizzleClickRepository(db)
});
