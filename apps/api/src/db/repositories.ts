...
class DrizzleCampaignOfferRepository implements CampaignOfferRepository {
  ...
  async listByCampaign(campaignId: string) {
    return this.db.select().from(campaignOffers).where(eq(campaignOffers.campaignId, campaignId));
  }

  async find(campaignId: string, affiliateOfferId: string) {
    const rows = await this.db.select().from(campaignOffers).where(and(eq(campaignOffers.campaignId, campaignId), eq(campaignOffers.affiliateOfferId, affiliateOfferId))).limit(1);
    return rows[0];
  }

  async save(entity: CampaignOffer) {
    await this.db.insert(campaignOffers).values(entity);
    return entity;
  }

  async remove(campaignId: string, affiliateOfferId: string) {
    await this.db.delete(campaignOffers).where(and(eq(campaignOffers.campaignId, campaignId), eq(campaignOffers.affiliateOfferId, affiliateOfferId)));
  }
}
