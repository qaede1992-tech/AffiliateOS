...
export const autonomousOptimizationStates = pgTable(
  "autonomous_optimization_states",
  {
...
const toDomain = (row: StateRow): OptimizationState => {
  if (!actions.has(row.action as OptimizationAction)) {
    throw new Error(`Unknown optimization action persisted for campaign ${row.campaignId}.`);
  }
...
export class DrizzleOptimizationStateReader implements OptimizationStateReader {
  ...
  async get(campaignId: string): Promise<OptimizationState | undefined> {
    const rows = await this.db.select().from(autonomousOptimizationStates)
      ...
      .orderBy(desc(autonomousOptimizationStates.updatedAt))
      .limit(1);
  ...
  async save(campaignId: string, state: OptimizationState): Promise<OptimizationState> {
    ...
    const rows = await this.db.insert(autonomousOptimizationStates)
      .values({ campaignId, action: state.action, appliedAt: state.appliedAt, createdAt: now, updatedAt: now })
      .onConflictDoUpdate({ target: autonomousOptimizationStates.campaignId, set: { action: state.action, appliedAt: state.appliedAt, updatedAt: now } })
      .returning();
