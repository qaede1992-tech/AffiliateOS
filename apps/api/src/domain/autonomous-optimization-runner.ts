...
import { OptimizationEngine, type OptimizationPolicy, type OptimizationRecommendation } from "./optimization-engine.js";
...
/**
 * Coordinates measurement, optimization decisions, guarded campaign actions,
 * and persistence of only actions that were actually applied.
 */
export class AutonomousOptimizationRunner {
  ...
  async run(now = new Date()): Promise<AutonomousOptimizationRunResult> {
    ...
    const state = new Map<string, import("./optimization-engine.js").OptimizationState>();
