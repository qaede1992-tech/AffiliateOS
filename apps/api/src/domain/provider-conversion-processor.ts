...
export class ProviderConversionProcessor {
  ...
  async process(accountScope: string, event: NormalizedProviderConversion): Promise<Conversion> {
    ...
    if (!affiliateReference) throw new DomainError("PROVIDER_CONVERSION_AFFILIATE_MISSING", "Provider conversion is missing an affiliate reference.", 422);
    ...
    if (!offerReference) throw new DomainError("PROVIDER_CONVERSION_OFFER_MISSING", "Provider conversion is missing an offer reference.", 422);
    ...
    const affiliateId = await this.resolver.resolveAffiliate(affiliateReference);
    ...
    const offerId = await this.resolver.resolveOffer(offerReference);
