...
type ExplorationGroupSignal = Pick<AutonomousExplorationState, "sampleCount" | "promotedCount" | "deprioritizedCount" | "optimizationPositiveCount" | "optimizationNegativeCount" | "regimeRisingCount" | "regimeStableCount" | "regimeDecliningCount" | "regimeVolatileCount">;
...
export class AdaptiveExplorationPolicyProvider {
 ...
 async getRates(candidates:OpportunityCandidateSource[],basePolicy:OpportunitySelectionPolicy,policiesByMarketplace:OpportunitySelectionPoliciesByMarketplace={},performance:Map<string,OpportunityPerformanceSignal>=new Map()):Promise<Map<string,number>>{
  ...
  if(this.stateRepository){await this.stateRepository.applyEvaluatedAudits(audits);await this.stateRepository.applyOptimizationFeedback?.();}
  ...
  else for(const a of audits){const e=a.outcome?.explorationEvaluation;if(a.selectionMode!=="exploration"||!e||!["promote-to-exploitation","deprioritize"].includes(e.status))continue;const add=(d:"marketplace"|"category"|"audience",v:string)=>{const k=key(a.marketplaceId,d,v),s=signals.get(k)??{sampleCount:0,promotedCount:0,deprioritizedCount:0,optimizationPositiveCount:0,optimizationNegativeCount:0,regimeRisingCount:0,regimeStableCount:0,regimeDecliningCount:0,regimeVolatileCount:0};s.sampleCount++;if(e.status==="promote-to-exploitation")s.promotedCount++;else s.deprioritizedCount++;signals.set(k,s);};add("marketplace",a.marketplaceId);if(a.category)add("category",a.category);for(const v of a.audienceSegments??[])add("audience",String(v));}
  ...
  if(exact?.anomalyRecovery === "recovering") {
    ...
    else if(exact.regime === "declining") rate=clamp(rate * (1 + 0.25 * recoveryConfidence), minRate, maxRate);
    else if(exact.regime === "volatile") rate=clamp(rate * (1 + 0.1 * recoveryConfidence), minRate, maxRate);
  ...
  }const cat=c.product.category?.trim(),cs=cat?signals.get(key(c.product.marketplaceId,"category",cat)):undefined;if(cs&&cs.sampleCount>=minSamples){rate=this.adjust(rate,cs,promote,deprioritize,reduce,increase,minRate,maxRate);specific=true;}if(cs&&(cs.optimizationPositiveCount+cs.optimizationNegativeCount)>=minSamples)rate=this.adjustOptimization(rate,cs,promote,deprioritize,reduce,increase,minRate,maxRate);if(!specific){for(const a of p.requiredAudience??[]){const as=signals.get(key(c.product.marketplaceId,"audience",String(a)));if(as&&as.sampleCount>=minSamples){rate=this.adjust(rate,as,promote,deprioritize,reduce,increase,minRate,maxRate);specific=true;if((as.optimizationPositiveCount+as.optimizationNegativeCount)>=minSamples)rate=this.adjustOptimization(rate,as,promote,deprioritize,reduce,increase,minRate,maxRate);break;}}}if(!specific){const m=signals.get(key(c.product.marketplaceId,"marketplace",c.product.marketplaceId));if(m&&m.sampleCount>=minSamples){rate=this.adjust(rate,m,promote,deprioritize,reduce,increase,minRate,maxRate);rate=this.adjustHistoricalRegime(rate,m,minRate,maxRate);if((m.optimizationPositiveCount+m.optimizationNegativeCount)>=minSamples)rate=this.adjustOptimization(rate,m,promote,deprioritize,reduce,increase,minRate,maxRate);}}rates.set(c.product.id,Math.round(rate * 100) / 100);}
  return rates;
 }
 private adjustOptimization(rate:number,s:ExplorationGroupSignal,p:number,d:number,r:number,i:number,min:number,max:number){const total=s.optimizationPositiveCount+s.optimizationNegativeCount;if(total===0)return rate;const pr=s.optimizationPositiveCount/total,dr=s.optimizationNegativeCount/total;if(pr>=p&&dr<d)return clamp(rate*r,min,max);if(dr>=d&&pr<p)return clamp(rate*i,min,max);return rate;}
 private adjustHistoricalRegime(rate:number,s:ExplorationGroupSignal,min:number,max:number){const total=(s.regimeRisingCount??0)+(s.regimeStableCount??0)+(s.regimeDecliningCount??0)+(s.regimeVolatileCount??0);if(total===0)return rate;const rising=(s.regimeRisingCount??0)/total,declining=(s.regimeDecliningCount??0)/total,volatileRate=(s.regimeVolatileCount??0)/total;return clamp(rate*(1-0.15*rising+0.2*declining+0.1*volatileRate),min,max);}
