...
export class ProductOpportunityService {
  score(product: Product, commissionRateBps = 0, audienceRelevance = 0): ProductOpportunity { const rating = Math.min(20, Math.round((product.ratingMilli ?? 0) / 250)); const reviews = Math.min(15, Math.round(Math.log10(product.reviewCount + 1) * 4)); const sales = Math.min(20, Math.round(Math.log10(product.soldCount + 1) * 5)); const commission = Math.min(30, Math.round(commissionRateBps / 333)); const audience = Math.min(15, Math.max(0, Math.round(audienceRelevance * 15))); const reasons = [`commission signal: ${commission}/30`, `rating signal: ${rating}/20`, `review signal: ${reviews}/15`, `sales signal: ${sales}/20`, `audience signal: ${audience}/15`]; return { product, score: commission + rating + reviews + sales + audience, reasons, disclaimer: "Opportunity score prioritizes available signals; it is not a prediction or guarantee of sales." }; }
...
export class SafeTemplateContentGenerator implements ContentGenerator { async generateSocialPost(product: Product, platform: GeneratedContent["platform"]): Promise<GeneratedContent> { const cta = "Check availability and terms through the affiliate link."; return { platform, title: product.name, caption: `${product.name}${product.category ? ` for ${product.category} shoppers` : ""}. ${cta}`, script: platform === "tiktok" || platform === "youtube-shorts" ? `Show ${product.name}. Highlight only the listed product details. ${cta}` : undefined, cta }; } generateProductContent(product: Product): Promise<GeneratedContent[]> { return Promise.all((["tiktok", "instagram", "facebook", "youtube-shorts", "x", "threads"] as const).map((platform) => this.generateSocialPost(product, platform))); } generateShortVideoScript(product: Product, platform: "tiktok" | "youtube-shorts"): Promise<GeneratedContent> { return this.generateSocialPost(product, platform); } generateCampaignContent(products: Product[]): Promise<GeneratedContent[]> { return Promise.all(products.map((product) => this.generateSocialPost(product, "instagram"))); } }
