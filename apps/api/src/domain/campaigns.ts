...
const generateCode = () => randomUUID().replaceAll("-", "").slice(0, 12);
...
function validateStatusTransition(current: Campaign["status"], next: Campaign["status"] | undefined) {
  if (!next || next === current) return;
  if (!allowedTransitions[current].includes(next)) {
    throw new DomainError("INVALID_CAMPAIGN_STATUS_TRANSITION", `Cannot change campaign status from ${current} to ${next}.`);
  }
}
...
export class CampaignService {
  ...
  async create(input: CreateCampaignRequest) {
    ...
    return this.campaigns.save({
      id: randomUUID(), name: input.name, objective: input.objective,
      status: input.status ?? "draft", startAt: input.startAt, endAt: input.endAt,
      audience: input.audience ?? {}, createdAt, updatedAt: createdAt
    });
  }
  ...
  async attachOffer(campaignId: string, affiliateOfferId: string) {
    ...
    if (!(await this.affiliateOffers.findById(affiliateOfferId))) {
      throw new DomainError("AFFILIATE_OFFER_NOT_FOUND", "The affiliate offer does not exist.", 404);
    }
    return (await this.campaignOffers.find(campaignId, affiliateOfferId)) ??
      this.campaignOffers.save({ campaignId, affiliateOfferId, createdAt: now() });
  }

  async listOffers(campaignId: string) {
    await this.get(campaignId);
    return this.campaignOffers.listByCampaign(campaignId);
  }

  async removeOffer(campaignId: string, affiliateOfferId: string) {
    await this.get(campaignId);
    await this.campaignOffers.remove(campaignId, affiliateOfferId);
  }
}

export class TrackingService {
  constructor(
    private readonly links: TrackingLinkRepository,
  ...
  async list(campaignId?: string) {
    if (!campaignId) return this.links.list();
    if (!(await this.campaigns.findById(campaignId))) {
      throw new DomainError("CAMPAIGN_NOT_FOUND", "The campaign does not exist.", 404);
    }
    return this.links.listByCampaign(campaignId);
  }
  ...
  async create(input: CreateTrackingLinkRequest) {
    ...
    if (input.campaignId) {
      ...
      if (!campaign) throw new DomainError("CAMPAIGN_NOT_FOUND", "The campaign does not exist.", 404);
      if (!(await this.campaignOffers.find(input.campaignId, input.affiliateOfferId))) {
        throw new DomainError("OFFER_NOT_ATTACHED", "The affiliate offer must be attached to the campaign first.");
      }
    }

    const trackingCode = input.code ?? generateCode();
    if (await this.links.findByCode(trackingCode)) {
      throw new DomainError("TRACKING_CODE_EXISTS", "The tracking code is already in use.", 409);
    }
    const createdAt = now();
    return this.links.save({
      id: randomUUID(), affiliateOfferId: input.affiliateOfferId, campaignId: input.campaignId,
      code: trackingCode, destinationUrl: input.destinationUrl, status: "active",
  ...
  async recordClick(id: string, input: RecordClickRequest) {
    ...
    return this.clicks.save({ id: randomUUID(), trackingLinkId: link.id, occurredAt: input.occurredAt ?? now(), metadata: input.metadata ?? {} });
  }

  async stats(id: string): Promise<TrackingLinkStats> {
    await this.get(id);
    return { linkId: id, clickCount: (await this.clicks.listByTrackingLink(id)).length };
  }
}
