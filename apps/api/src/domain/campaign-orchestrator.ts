...
export class CampaignOrchestrator {
  ...
  async execute(input: CampaignOrchestratorInput): Promise<CampaignOrchestrationResult> {
    ...
    if (input.offer.affiliateLinkExpiresAt) { const expiry = Date.parse(input.offer.affiliateLinkExpiresAt); if (!Number.isFinite(expiry) || expiry <= Date.now()) throw new Error("Campaign orchestration requires a non-expired affiliate link."); }
