...
export type ScoredOpportunity = ProductOpportunity & {
...
const audienceCategories: Record<AudienceSegment, string[]> = {
  beauty: ["beauty", "makeup", "cosmetic", "skincare"],
  skincare: ["skincare", "skin", "serum", "moisturizer", "cosmetic"],
  baby: ["baby", "infant", "newborn", "diaper"],
  parenting: ["parent", "parenting", "baby", "family"],
  fashion: ["fashion", "clothing", "apparel", "shoes", "dress"],
  home: ["home", "decor", "furniture", "storage"],
  kitchen: ["kitchen", "cook", "cooking", "bake", "utensil"],
  electronics: ["electronic", "phone", "laptop", "gadget", "computer"],
  lifestyle: ["lifestyle", "wellness", "fitness", "travel"],
  "deal-hunters": ["deal", "discount", "sale", "promo", "bundle"]
};
...
function audienceFit(product: Product, audience: AudienceSegment[]): { score: number; matched: AudienceSegment[] } {
  if (audience.length === 0) return { score: 50, matched: [] };
  const text = textFor(product);
  const matched = audience.filter((segment) => audienceCategories[segment].some((term) => text.includes(term)));
...
export function scoreOpportunity(input: OpportunityScoringInput): ScoredOpportunity {
  ...
  const discount = product.originalPriceCents && product.originalPriceCents > product.priceCents
    ? clamp(((product.originalPriceCents - product.priceCents) / product.originalPriceCents) * 100)
    : 0;
  const targetPrice = input.targetPriceMaxCents && input.targetPriceMaxCents > 0
    ? clamp((1 - product.priceCents / input.targetPriceMaxCents) * 100)
    : 50;
  const priceAppeal = clamp(discount * 0.6 + targetPrice * 0.4);
  const availability = offer ? (offer.availability === "limited" ? 55 : 100) : 0;
  ...
  if (commission >= 60) reasons.push("Strong commission potential");
  if (demand >= 60) reasons.push("Strong demand signals from sales and reviews");
  if (audience.matched.length) reasons.push(`Matches ${audience.matched.join(", ")} audience intent`);
  if (priceAppeal >= 60) reasons.push("Competitive price or discount signal");
  if (socialProof >= 70) reasons.push("Strong rating and review evidence");
  if (!offer) reasons.push("No active affiliate offer available");
  if (availability < 100 && availability > 0) reasons.push("Offer availability is limited");
  ...
  return {
    ...
    breakdown: {
      ...
      audienceFit: Math.round(audience.score * 100) / 100,
      socialProof: Math.round(socialProof * 100) / 100,
      priceAppeal: Math.round(priceAppeal * 100) / 100,
      availability,
      confidencePenalty,
...
export function rankOpportunities(inputs: OpportunityScoringInput[]): ScoredOpportunity[] {
  return inputs
