...
function bestOffer(productId: string, offers: AffiliateOffer[]): AffiliateOffer | undefined { return offers.filter((offer) => offer.productId === productId && offer.status === "active" && offer.affiliateLinkStatus === "active" && Boolean(offer.affiliateUrl) && offer.availability !== "out_of_stock").slice().sort((a, b) => (b.commissionRateBps ?? 0) - (a.commissionRateBps ?? 0) || (b.commissionAmountCents ?? 0) - (a.commissionAmountCents ?? 0) || a.id.localeCompare(b.id))[0]; }
