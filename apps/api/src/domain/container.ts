...
import { AutonomousCycleService, type AutonomousCycleInput } from "./autonomous-cycle.js";
...
import { InMemoryAutonomousOptimizationStateRepository, type AutonomousOptimizationStateRepository } from "./autonomous-optimization-state.js";
import { InMemoryAutonomousCycleLockRepository, type AutonomousCycleLockRepository } from "./autonomous-cycle-lock.js";
...
export function createServices(repositories: RepositorySet, transactionManager: TransactionManager, marketplaceRegistry = new MarketplaceProviderRegistry(), socialOAuthRegistry = new InMemorySocialOAuthProviderRegistry(), oauthStateRepository: OAuthStateRepository = new InMemoryOAuthStateRepository(), analyticsReader?: AnalyticsReader, attributionRepository: ConversionAttributionRepository = new InMemoryConversionAttributionRepository(), socialPublishers: SocialPublisher[] = [], socialCredentialResolver?: SocialCredentialResolver, publicationOperationRepository: PublicationOperationRepository = new InMemoryPublicationOperationRepository(), autonomousRunRepository: AutonomousRunRepository = repositories.autonomousRuns ?? new InMemoryAutonomousRunRepository(), autonomousSchedulerIntervalMs?: number, autonomousFeedbackMemoryRepository: AutonomousFeedbackMemoryRepository = new InMemoryAutonomousFeedbackMemoryRepository(), autonomousOptimizationStateRepository: AutonomousOptimizationStateRepository = repositories.autonomousOptimizationStates ?? new InMemoryAutonomousOptimizationStateRepository(), autonomousCycleLock: AutonomousCycleLockRepository = repositories.autonomousCycleLock ?? new InMemoryAutonomousCycleLockRepository(), autonomousCycleInput: AutonomousCycleInput = {}): Services {
  ...
  const campaignOrchestrator = new CampaignOrchestrator(campaigns, tracking, content, undefined, distribution, autonomousRuns, marketplace);
  ...
  const autonomousCycle = new AutonomousCycleService(candidateProvider, autonomousExecution, analytics, undefined, campaigns, content, autonomousOptimizationStateRepository, autonomousCycleLock);
