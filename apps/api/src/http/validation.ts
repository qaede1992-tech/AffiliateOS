...
export const createConversionSchema = z.object({ affiliateId: entityId, offerId: entityId, amountCents: z.number().int().positive().max(2147483647), occurredAt: isoTimestamp.optional(), idempotencyKey: z.string().trim().min(8).max(200).optional() });
...
export const createMarketplaceConnectionSchema = z.object({ name: nonEmptyText.max(100), slug: z.string().trim().regex(/^[a-z0-9][a-z0-9-]{0,99}$/), providerSlug: z.string().trim().regex(/^[a-z0-9][a-z0-9-]{0,99}$/), credentialReference: z.string().trim().min(3).max(255).optional(), configuration: safeConfiguration, enabled: z.literal(false).default(false) });
...
export const marketplaceEnableSchema = z.discriminatedUnion("enabled", [z.object({ enabled: z.literal(false) }), z.object({ enabled: z.literal(true), confirmation: z.literal("CONFIRM_MARKETPLACE_CONNECTION") })]);
