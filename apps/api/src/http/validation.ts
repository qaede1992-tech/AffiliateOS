...
export const scoreProductSchema = z.object({ product: z.object({ id: entityId, marketplaceId: entityId, externalProductId: nonEmptyText.max(255), name: nonEmptyText.max(500), description: z.string().max(10000).optional(), category: z.string().max(255).optional(), priceCents: z.number().int().nonnegative(), originalPriceCents: z.number().int().nonnegative().optional(), currency: z.string().length(3), ratingMilli: z.number().int().min(0).max(5000).optional(), reviewCount: z.number().int().nonnegative(), soldCount: z.number().int().nonnegative(), imageUrl: z.string().url().optional(), productUrl: z.string().url(), status: z.enum(["active", "inactive", "archived"]), createdAt: isoTimestamp, updatedAt: isoTimestamp }), commissionRateBps: z.number().int().min(0).max(10000).default(0), audienceRelevance: z.number().min(0).max(1).default(0) });
...
export const createCampaignSchema = z.object({ name: nonEmptyText.max(200), objective: nonEmptyText.max(100), status: z.enum(["draft", "scheduled", "active", "paused", "completed", "archived"]).default("draft"), startAt: isoTimestamp.optional(), endAt: isoTimestamp.optional(), audience: z.record(z.string(), z.unknown()).default({}) }).refine((value) => !value.startAt || !value.endAt || new Date(value.startAt).getTime() <= new Date(value.endAt).getTime(), "startAt must be before endAt");
export const updateCampaignSchema = z.object({ name: nonEmptyText.max(200).optional(), objective: nonEmptyText.max(100).optional(), status: z.enum(["draft", "scheduled", "active", "paused", "completed", "archived"]).optional(), startAt: isoTimestamp.optional(), endAt: isoTimestamp.optional(), audience: z.record(z.string(), z.unknown()).optional() }).refine((value) => Object.keys(value).length > 0, "At least one campaign field is required.");
