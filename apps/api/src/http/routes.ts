...
export function registerResourceRoutes(app: FastifyInstance, services: Services): void {
  ...
  app.post<{ Body: CreateConversionRequest; Reply: Conversion }>("/api/v1/conversions", writeGuard, async (request, reply) => reply.status(201).send(await services.conversions.create(createConversionSchema.parse(request.body))));
  app.post("/api/v1/conversions/:conversionId/attribution", writeGuard, async (request, reply) => { const params = conversionIdSchema.parse(request.params); const attribution = await services.attribution.create(params.conversionId, createConversionAttributionSchema.parse(request.body)); return reply.status(201).send(attribution); });
  ...
  app.post<{ Body: { product: Product; commissionRateBps?: number; audienceRelevance?: number }; Reply: ProductOpportunity }>("/api/v1/product-opportunities/score", writeGuard, async (request) => { const input = scoreProductSchema.parse(request.body); return new ProductOpportunityService().score(input.product, input.commissionRateBps, input.audienceRelevance); });
  ...
  app.patch("/api/v1/marketplaces/:connectionSlug", writeGuard, async (request) => { const connection = await services.marketplace.updateConnection(marketplaceSlugSchema.parse(request.params).connectionSlug, updateMarketplaceConnectionSchema.parse(request.body)); request.log.info({ event: "marketplace_connection_updated", connectionSlug: connection.slug, providerSlug: connection.providerSlug }); return connection; });
  app.put("/api/v1/marketplaces/:connectionSlug/enabled", writeGuard, async (request) => { const input = marketplaceEnableSchema.parse(request.body); const connection = await services.marketplace.setEnabled(marketplaceSlugSchema.parse(request.params).connectionSlug, input.enabled, "confirmation" in input ? input.confirmation : undefined); auditSecurityEvent(request.log, request, "marketplace_connection_enabled_changed", { connectionSlug: connection.slug, enabled: connection.enabled }); return connection; });
  app.post("/api/v1/marketplaces/:connectionSlug/test", writeGuard, async (request) => { const connection = await services.marketplace.testConnection(marketplaceSlugSchema.parse(request.params).connectionSlug); request.log.info({ event: "marketplace_connection_tested", connectionSlug: connection.slug, healthStatus: connection.healthStatus }); return connection; });
  ...
  app.post("/api/v1/marketplaces/:connectionSlug/products/discover", writeGuard, async (request) => { const { connectionSlug } = marketplaceSlugSchema.parse(request.params); return list(await services.marketplace.discoverProducts(connectionSlug)); });
  ...
  app.post("/api/v1/marketplaces/:connectionSlug/products/:externalProductId/offers/:externalOfferId/affiliate-link", writeGuard, async (request) => { const input = marketplaceLinkSchema.parse(request.params); return services.marketplace.generateAffiliateLink(input.connectionSlug, input.externalProductId, input.externalOfferId); });
  app.get("/api/v1/campaigns", async () => list(await services.campaigns.list()));
  app.post("/api/v1/campaigns", writeGuard, async (request, reply) => reply.status(201).send(await services.campaigns.create(createCampaignSchema.parse(request.body))));
  ...
  app.patch("/api/v1/campaigns/:campaignId", writeGuard, async (request) => services.campaigns.update(campaignIdSchema.parse(request.params).campaignId, updateCampaignSchema.parse(request.body)));
  ...
  app.post("/api/v1/campaigns/:campaignId/offers/:affiliateOfferId", writeGuard, async (request) => { const params = campaignOfferParamsSchema.parse(request.params); return services.campaigns.attachOffer(params.campaignId, params.affiliateOfferId); });
  app.delete("/api/v1/campaigns/:campaignId/offers/:affiliateOfferId", writeGuard, async (request) => { const params = campaignOfferParamsSchema.parse(request.params); await services.campaigns.removeOffer(params.campaignId, params.affiliateOfferId); return { status: "removed" }; });
  ...
  app.post("/api/v1/tracking-links", writeGuard, async (request, reply) => reply.status(201).send(await services.tracking.create(createTrackingLinkSchema.parse(request.body))));
  ...
  app.post("/api/v1/tracking-links/:trackingLinkId/clicks", writeGuard, async (request) => services.tracking.recordClick(trackingLinkIdSchema.parse(request.params).trackingLinkId, recordClickSchema.parse(request.body)));
  ...
  app.post("/api/v1/content", writeGuard, async (request, reply) => reply.status(201).send(await services.content.create(createContentSchema.parse(request.body))));
  app.get("/api/v1/content/:contentId", async (request) => services.content.get(contentIdSchema.parse(request.params).contentId));
  app.patch("/api/v1/content/:contentId", writeGuard, async (request) => services.content.update(contentIdSchema.parse(request.params).contentId, updateContentSchema.parse(request.body)));
  app.get("/api/v1/social-accounts", async () => list(await services.socialAccounts.list()));
  app.post("/api/v1/social-accounts", writeGuard, async (request, reply) => reply.status(201).send(await services.socialAccounts.create(createSocialAccountSchema.parse(request.body))));
