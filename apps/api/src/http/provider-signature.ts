...
export type ProviderSignatureOptions = {
  /** Maximum accepted age for a signed request, in milliseconds. */
  maxAgeMs?: number;
  /** Allow a small amount of future clock skew. */
  futureSkewMs?: number;
};
...
/**
 * Verify an HMAC-SHA256 provider event signature.
 *
 * The signed value is `${timestamp}.${rawBody}`. The timestamp is deliberately
 * part of the authenticated material so a valid body cannot simply be replayed
 * outside the accepted time window.
 */
export function verifyProviderEventSignature(
  rawBody: string,
  secret: string,
  headers: ProviderSignatureHeaders,
  nowMs = Date.now(),
...
/**
 * Small bounded replay guard for a single process. Persistent/distributed
 * deployments should back this boundary with shared storage before scaling
