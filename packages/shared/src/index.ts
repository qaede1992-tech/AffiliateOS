...
export interface Conversion { id: EntityId; affiliateId: EntityId; offerId: EntityId; amountCents: MoneyCents; status: ConversionStatus; occurredAt: IsoTimestamp; }
...
export interface Product { id: EntityId; marketplaceId: EntityId; externalProductId: string; name: string; description?: string; category?: string; priceCents: number; originalPriceCents?: number; currency: string; ratingMilli?: number; reviewCount: number; soldCount: number; imageUrl?: string; productUrl: string; status: "active" | "inactive" | "archived"; createdAt: IsoTimestamp; updatedAt: IsoTimestamp; }
...
export interface MarketplaceConnection { id: EntityId; name: string; slug: string; providerSlug: string; status: MarketplaceConnectionStatus; connectionMode: "mock" | "official_api"; enabled: boolean; credentialReference?: string; configuration: Record<string, unknown>; healthStatus: MarketplaceConnectionHealth; healthError?: string; healthMetadata: Record<string, unknown>; lastCheckedAt?: IsoTimestamp; lastSuccessfulCheckAt?: IsoTimestamp; lastSuccessfulSyncAt?: IsoTimestamp; createdAt: IsoTimestamp; updatedAt: IsoTimestamp; }
...
export interface Click { id: EntityId; trackingLinkId: EntityId; idempotencyKey?: string; occurredAt: IsoTimestamp; metadata: Record<string, unknown>; }
...
export interface RecordClickRequest { occurredAt?: IsoTimestamp; idempotencyKey?: string; metadata?: Record<string, unknown>; }

export type ContentPlatform = "tiktok" | "instagram" | "facebook" | "youtube-shorts" | "x" | "threads";
export type ContentStatus = "draft" | "scheduled" | "published" | "failed" | "archived";
export interface Content { id: EntityId; productId?: EntityId; campaignId?: EntityId; platform: ContentPlatform; contentType: string; title?: string; caption?: string; script?: string; cta?: string; status: ContentStatus; scheduledAt?: IsoTimestamp; publishedAt?: IsoTimestamp; createdAt: IsoTimestamp; updatedAt: IsoTimestamp; }
export interface CreateContentRequest { productId?: EntityId; campaignId?: EntityId; platform: ContentPlatform; contentType: string; title?: string; caption?: string; script?: string; cta?: string; status?: ContentStatus; scheduledAt?: IsoTimestamp; publishedAt?: IsoTimestamp; }
export interface UpdateContentRequest { productId?: EntityId; campaignId?: EntityId; platform?: ContentPlatform; contentType?: string; title?: string; caption?: string; script?: string; cta?: string; status?: ContentStatus; scheduledAt?: IsoTimestamp; publishedAt?: IsoTimestamp; }
...
export interface CreateSocialAccountRequest { platform: string; accountReference: string; status?: SocialAccountStatus; connection?: Record<string, unknown>; credentialReference?: string; }
